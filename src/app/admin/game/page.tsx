...
export default function GamePage() {
  ...
  // Team assignment state for idle screen
  ...
  const fetchContestants = useCallback(async () => {
    try {
      ...
      if (res.ok) {
        const data = await res.json()
        setContestants(data)
        // Initialize team assignments from current contestant data
  ...
  async function handleStartQuestion() {
    // Pick from the opposing team (the team being asked about)
    ...
    // Combined: set wager and start question in one call
  ...
  // Team assignment helpers for idle state
      ...
      <div className="p-4 max-w-5xl mx-auto">
        ...
        {(!session || session.status === 'idle') && (
          <div className="flex flex-col items-center gap-6 py-4">
              ...
              <div className="grid grid-cols-3 gap-3">
                ...
                <div className="bg-green-900/20 border border-green-600/40 rounded-2xl p-3">
                  ...
                  <div className="space-y-1 min-h-[60px]">
                    ...
                    {teamAIds.length === 0 && (
                      <div className="text-slate-600 text-xs text-center py-3">اضغط على اسم لإضافته</div>
                    )}
                  </div>
                </div>
                ...
                <div className="bg-slate-800/60 border border-slate-600/40 rounded-2xl p-3">
                  ...
                  <div className="space-y-1 min-h-[60px]">
                    {unassignedContestants.map(c => (
                      <div key={c.id} className="bg-slate-700/60 border border-slate-600/30 rounded-lg px-2 py-1.5">
                        ...
                        <div className="flex gap-1">
                          <button
                            onClick={() => assignToTeam(c.id, 'A')}
                            className="flex-1 text-xs bg-green-800/60 hover:bg-green-700 text-green-300 rounded py-0.5 transition-colors"
                          >
                            ← أ
                          </button>
                          <button
                            onClick={() => assignToTeam(c.id, 'B')}
                            className="flex-1 text-xs bg-blue-800/60 hover:bg-blue-700 text-blue-300 rounded py-0.5 transition-colors"
                          >
                            ب →
                          </button>
                        </div>
                    ...
                    {unassignedContestants.length === 0 && (
                      <div className="text-slate-600 text-xs text-center py-3">جميع المتسابقين وزّعوا</div>
                    )}
                  </div>
                </div>
            ...
            <button
              onClick={handleStart}
              disabled={actionLoading || teamAIds.length === 0 || teamBIds.length === 0}
          ...
          <div className="space-y-6">
            ...
            <div className="bg-slate-800/60 border border-slate-700 rounded-2xl p-5">
              ...
              <button
                ...
                disabled={actionLoading}
                className="w-full bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-500 hover:to-purple-500 text-white font-bold py-4 rounded-xl transition-colors text-xl"
              >
                ابدأ السؤال ▶ ({selectedWager} نقطة)
              </button>
            </div>
          </div>
        )}
            ...
            <div className="bg-gradient-to-r from-indigo-900/80 to-purple-900/80 border border-indigo-500/50 rounded-2xl p-5">
              ...
              <div className="text-center">
                ...
                <p className="text-yellow-300 text-xl font-bold mb-3">
                  {st?.contestant_name || currentContestantName || '—'}
                </p>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="space-y-2">
                ...
                <button
                  ...
                  disabled={actionLoading || helplineUsed}
                ...
                >
                  ...
                  <div className="text-xs opacity-70 mt-0.5">
                    نفس المتسابق • {settings.helpline_same_person.cost} نقطة • ↓{Math.round(settings.helpline_same_person.multiplier_reduction * 100)}%
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-xs opacity-70 mt-0.5">
                    {settings.helpline_opposing_team.cost} نقطة • ↓{Math.round(settings.helpline_opposing_team.multiplier_reduction * 100)}%
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-xs opacity-70 mt-0.5">
                    {settings.helpline_wild.cost} نقطة • ↓{Math.round(settings.helpline_wild.multiplier_reduction * 100)}%
                  </div>
                </button>
                ...
                >
                  ...
                  <div className="text-xs opacity-70 mt-0.5">
                    {settings.helpline_remove_two.cost} نقطة • ↓{Math.round(settings.helpline_remove_two.multiplier_reduction * 100)}%
                  </div>
                </button>

                {helplineUsed && (
                  <div className="text-center text-slate-500 text-xs mt-1">تم استخدام المساعدة لهذا السؤال</div>
                )}
              </div>
            </div>
          </div>
